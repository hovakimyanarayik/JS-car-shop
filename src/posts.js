export default class Posts {
  static getPosts() {
    return fetch(
      "https://carhouse-yerevan-default-rtdb.firebaseio.com/posts.json"
    )
  ...
  static async filterAndRenderByModel(model) {
    ...
    posts = posts.filter((post) =>
      post.model.toLowerCase().includes(model.toLowerCase())
    );
    return postsToHTML(posts);
  }
  ...
  static createPost(
    ...
    return fetch(
      `https://carhouse-yerevan-default-rtdb.firebaseio.com/posts.json?auth=${localStorage.getItem(
        "token"
      )}`,
      {
        method: "POST",
  ...
  static getUserPosts() {
    ...
    return Posts.getPosts().then((response) => {
      if (!response) return;
  ...
  static removePostById(id) {
    ...
    return fetch(
      `https://carhouse-yerevan-default-rtdb.firebaseio.com/posts/${id}.json?auth=${localStorage.getItem(
        "token"
      )}`,
      {
        method: "DELETE",
    ...
    .map(
      (post) => `
            <div class="post">
                <img src="${
                  post.img ||
                  "https://prestigemotorsport.com.au/wp-content/uploads/car_no_image_small.jpg"
                }" alt="image">
                <div class="car-info">
                    ...
                    ${
                      user
                        ? `<button class="remove-btn" data-action="remove-ad" data-id="${post.id}">Remove</button>`
                        : ""
                    }
                </div>
            </div>
